...
export const ERROR_MESSAGES: Record<DexErrorCode, string> = {
  ...
  NO_POOL: 'No verified on-chain liquidity pool found for this token pair on the selected network.',
  RPC_ERROR: 'Blockchain RPC node returned an error while processing request.',
  RPC_TIMEOUT: 'Blockchain RPC request timed out while communicating with the node.',
  RPC_RATE_LIMITED: 'Blockchain RPC rate limit reached. Please wait a moment.',
  RPC_UNAVAILABLE: 'Unable to connect to the blockchain RPC node. Please try again or switch networks.',
  INVALID_POOL: 'Liquidity pool configuration or on-chain state is invalid or corrupted.',
  INSUFFICIENT_LIQUIDITY: 'Pool exists but depth or active liquidity is insufficient for this trade size.',
  ORACLE_UNAVAILABLE: 'Price oracle consensus could not be established from independent sources.',
  SECURITY_BLOCKED: 'Transaction blocked due to honeypot or malicious security risk detected on token contract.',
  CHAIN_MISMATCH: 'Network chain ID mismatch between request payload and target contract or quote.',
  TOKEN_MISMATCH: 'Token address or symbol mismatch between route quote and execution payload.',
